...
export default function SportsCategoryPage() {
  ...
  useEffect(() => {
    ...
    async function loadProducts() {
      ...
      setError(null);
      try {
        const data = await getProductsByCategory("sports");
      ...
      <div className="min-h-screen bg-white max-md:bg-white pb-20 md:pb-8">
        <div className="max-w-7xl mx-auto px-4 py-6 md:py-8">
          <div className="mb-6 md:mb-8">
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Sports Collection</h1>
            <p className="text-gray-600 mt-2 text-sm md:text-base">
              Performance and athleisure for every day
            </p>
          </div>
          {children}
        </div>
